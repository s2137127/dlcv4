...
if __name__ == '__main__':
    ...
    resnet.load_state_dict(torch.load('./myresnet.pt'))
    ...
    optimizer = optim.Adam(fine.parameters(), lr=3e-4)
    ...
    train_ld,val_ld = DataLoader(train_ds, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=True,pin_memory=True),\
                      DataLoader(val_ds, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=True,pin_memory=True)
    best_acc = 0
    cnt = 0
    for ep in range(epoch):
        ...
        with torch.no_grad():
            ...
            if acc > best_acc:
                cnt = 0
                best_acc = acc
                torch.save(fine.state_dict(), './fine/fine_best_C.pth')
            elif best_acc > acc:
                cnt += 1
